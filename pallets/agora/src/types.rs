...
pub struct Job<T: crate::pallet::Config> {
...
#[derive(Encode, Decode, Clone, PartialEq, Eq, RuntimeDebug, TypeInfo, MaxEncodedLen)]
#[scale_info(skip_type_params(T))]
pub struct WorkerInfo<T: crate::pallet::Config> {
...
#[derive(Encode, Decode, Clone, PartialEq, Eq, RuntimeDebug, TypeInfo, MaxEncodedLen)]
#[scale_info(skip_type_params(T))]
pub struct Commit<T: crate::pallet::Config> {
    /// Worker who made the commit
    pub worker: <T as frame_system::Config>::AccountId,
    /// Salt used in the commit (32 bytes)
    pub salt: [u8; 32],
    /// Hash of salt + result (prevents preimage grinding)
    pub result_hash: <T as frame_system::Config>::Hash,
...
#[derive(Encode, Decode, Clone, PartialEq, Eq, RuntimeDebug, TypeInfo, MaxEncodedLen)]
#[scale_info(skip_type_params(T))]
pub struct Reveal<T: crate::pallet::Config> {
    /// Worker who revealed
    pub worker: <T as frame_system::Config>::AccountId,
    /// Salt used in the commit (32 bytes)
    pub salt: [u8; 32],
...
#[derive(Encode, Decode, Clone, PartialEq, Eq, RuntimeDebug, TypeInfo, MaxEncodedLen)]
#[scale_info(skip_type_params(T))]
pub struct OCWExecutionState<T: crate::pallet::Config> {
    /// Job ID
    pub job_id: JobId,
    /// Execution status
